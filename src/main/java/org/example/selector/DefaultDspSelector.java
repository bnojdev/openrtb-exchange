...
public class DefaultDspSelector implements DspSelector{
    ...
    public List<Dsp> select(BidRequest bidRequest, List<Dsp> dspRegister) {
        return dspRegister.stream()
                .filter(dsp -> dsp.getSupportedAdTypes().contains(bidRequest.getPlacement().getAdType()))
                .filter(dsp -> dsp.getSupportedCountries().contains(bidRequest.getPlacement().getCountry()))
                .toList();
    }
}
